(function() {
  ...
  /* ---- Telegram Config ---- */
  const BOT_TOKEN = '8680166732:AAHfiNSC7piFugqIo-eUGsZfXjd1C0AXxpw';
  const CHAT_ID   = '5736685592';
  ...
  function spawnEmote() {
    /* Hanya aktif di gate3, dan tidak di slide 1 (scroll < 80px) */
    if (!gate3.classList.contains('active')) return;
    if (gate3.scrollTop < 80) return;
  ...
  /* ==============================================================
     3. GATE 2 → TRANSITION VIDEO → GATE 3
        Urutan: fade overlay hitam → play prewed1.mp4 → fade out video → gate3 aktif
  ============================================================== */
  const prewedTransition = document.getElementById('prewed-transition');
  const prewedVideo      = document.getElementById('prewed-video');
  const prewedOverlay    = document.getElementById('prewed-fade-overlay');

  function showPrewedTransition(onDone) {
    /* Jika file video tidak ada / elemen tidak ada, langsung skip */
    if (!prewedTransition || !prewedVideo) {
      onDone();
      return;
    }

    /* Tampilkan container video, mulai dari gelap (opacity 0 via CSS) */
    prewedTransition.style.display  = 'flex';
    prewedTransition.style.opacity  = '0';
    prewedVideo.style.opacity       = '0';
    if (prewedOverlay) prewedOverlay.style.opacity = '1';

    /* Fade IN container */
    requestAnimationFrame(function() {
      prewedTransition.style.transition = 'opacity 0.5s ease';
      prewedTransition.style.opacity    = '1';
    });

    /* Setelah fade-in selesai (~500ms), tampilkan video & play */
    setTimeout(function() {
      prewedVideo.currentTime = 0;
      if (prewedOverlay) {
        prewedOverlay.style.transition = 'opacity 0.6s ease';
        prewedOverlay.style.opacity    = '0';
      }
      prewedVideo.style.transition = 'opacity 0.6s ease';
      prewedVideo.style.opacity    = '1';

      var playPromise = prewedVideo.play();
      var videoHandled = false;

      /* Durasi video atau max 6 detik, lalu fade-out */
      function beginFadeOut() {
        if (videoHandled) return;
        videoHandled = true;

        /* Fade OUT video */
        prewedVideo.style.transition   = 'opacity 0.7s ease';
        prewedVideo.style.opacity      = '0';
        if (prewedOverlay) {
          prewedOverlay.style.opacity  = '1';
        }
        prewedTransition.style.transition = 'opacity 0.7s ease';

        setTimeout(function() {
          prewedTransition.style.opacity = '0';
          setTimeout(function() {
            prewedTransition.style.display = 'none';
            prewedVideo.pause();
            onDone();
          }, 750);
        }, 700);
      ...
      /* Dengarkan event ended dari video */
      prewedVideo.addEventListener('ended', beginFadeOut, { once: true });

      /* Fallback: jika video lebih dari 8 detik atau gagal play */
      if (playPromise) {
        playPromise.catch(function() {
          /* Video gagal play → langsung onDone setelah 1 detik */
          setTimeout(function() {
            prewedTransition.style.display = 'none';
            onDone();
          }, 1000);
  ...
  btnOpen.addEventListener('click', function () {
    ...
    setTimeout(function () {
      /* Tampilkan transition video, lalu aktifkan gate3 setelah selesai */
      showPrewedTransition(function() {
        ...
        /* Petal rain dimatikan karena #petal-layer sudah dihapus dari HTML */
        /* startPetalRain(); */
  ...
  function showToast(msg) {
    ...
    Swal.fire({
      ...
      color:            isDark ? '#e8e0db' : '#3a2e2a',
      backdrop:         'rgba(0,0,0,0.38)',
      customClass: {
        popup:            'swal-glass',
        title:            'swal-glass-title',
        timerProgressBar: 'swal-glass-progress'
      },
      width:   '280px',
      padding: '1.6em'
    });
  ...
  /* ==============================================================
     16. UPLOAD BUKTI TRANSFER — preview + kirim ke Telegram
         via sendDocument (multipart/form-data)
  ============================================================== */
  var buktiFile = null;

  window.handleBuktiUpload = function(input) {
    if (!input.files || !input.files[0]) return;
    buktiFile = input.files[0];

    var reader = new FileReader();
    reader.onload = function(e) {
      var previewImg  = document.getElementById('bukti-preview-img');
      var previewWrap = document.getElementById('bukti-preview-wrap');
      var statusEl    = document.getElementById('bukti-send-status');
  ...
  window.sendBuktiTelegram = function() {
    if (!buktiFile) return;

    var sendBtn  = document.getElementById('bukti-send-btn');
    var statusEl = document.getElementById('bukti-send-status');
    ...
    formData.append('caption',
      '📎 *Bukti Transfer — Adib & Nabila*\n\n' +
      '👤 ' + (guestName || 'Tamu') + '\n' +
      '🕐 ' + new Date().toLocaleString('id-ID')
    );
    formData.append('parse_mode', 'Markdown');
    formData.append('document',   buktiFile, buktiFile.name);
    ...
    .then(function(data) {
      if (data.ok) {
        if (statusEl) statusEl.textContent = '✓ Terkirim! Terima kasih 🤍';
        if (sendBtn)  sendBtn.textContent  = 'Terkirim ✓';
        buktiFile = null;
      } else {
    ...
    .catch(function() {
      if (statusEl) statusEl.textContent = '❌ Gagal kirim. Coba lagi.';
      if (sendBtn) {
        sendBtn.disabled    = false;
        sendBtn.textContent = 'Kirim ke Panitia ✦';
      }
    });
  };
  ...
  /* ==============================================================
     E-2. GALLERY SWIPER — Swiper.js slider prewedding (Slide 4)
          Loop + coverflow + autoplay + pagination + navigation
          Klik foto → buka lightbox (jika tersedia)
  ============================================================== */
  function initGallerySwiper() {
    ...
    new Swiper('.gallery-swiper', {
      ...
      /* Efek coverflow — foto "melayang" seperti album foto */
      effect: 'coverflow',
      coverflowEffect: {
        rotate:       18,
        stretch:      0,
      ...
      /* Autoplay lambat supaya bisa dinikmati */
      autoplay: {
        delay:                3800,
      ...
      /* Keyboard supaya bisa dikontrol panah keyboard di desktop */
      ...
      on: {
        click: function(swiper, event) {
          ...
          if (img && img.classList.contains('gallery-swiper-img')) {
      ...
      mutations.forEach(function(mut) {
        if (mut.attributeName === 'class' &&
          ...
          setTimeout(function() {
            ...
            /* ── AOS: Animate On Scroll (ganti IntersectionObserver manual) ── */
            ...
            initGallerySwiper();    /* E-2: swiper galeri foto prewedding */
